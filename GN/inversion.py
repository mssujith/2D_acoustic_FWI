...
n1 = 8
for i1 in range(0, nz, n1):
    for i2 in range(0, nx, n1):
        data1[i1:i1+n1, i2:i2+n1] = np.mean(data[i1:i1+n1, i2:i2+n1])

n1 = 3
for i1 in range(0, nz, n1):
    for i2 in range(0, nx, n1):
        data2[i1:i1+n1, i2:i2+n1] = np.mean(data[i1:i1+n1, i2:i2+n1])
...
while it <= 9:
    ...
    J, pred = forward_and_jacobian(F, Ki, src, Ts, Tr, dx, dz, n_pml, bi, bj)
    ...
    lamda = np.max(np.sum((np.matmul(J.T, (Wd.T * (Wd * J)))), axis=1)) / (it+1) * 10**-1
...
plot_models(V_init, V_upd, V_tru, n_pml, extent, vmax, vmin, figsize, aspect = 'equal', cmap = 'Spectral')
