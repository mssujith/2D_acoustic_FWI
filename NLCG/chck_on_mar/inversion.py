...
mod1 = np.loadtxt('./marmousi.dat')

data1 = np.flip(mod1, axis = [0])
data1 = data1[:80, 100:300]
...
offset_max = (nx - 2*n_pml) * dx
offset_max = int(offset_max)

z_src =  5
x_src = np.arange(50, offset_max, 50)

x_rec = np.arange(12.5, offset_max, 12.5)
z_rec = 10
...
while it <= 80:
    ...
    beeta = np.dot(gn.T, (gn - g0)) / np.dot(g0.T, g0) * 1
